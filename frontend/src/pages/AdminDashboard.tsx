...
// ─── Google Login Toggle Component ──────────────────────────────────────────
const GoogleLoginToggle = ({
  getBool, toggleBool, settingsLoading
}: {
  getBool: (key: any, defaultVal?: boolean) => boolean;
  toggleBool: (key: any, currentVal: boolean) => Promise<void>;
  settingsLoading: boolean;
}) => {
  const isEnabled = getBool(SITE_KEYS.GOOGLE_LOGIN_ENABLED, false); // Default OFF
  const [toggling, setToggling] = useState(false);

  const handleToggle = async () => {
    setToggling(true);
    await toggleBool(SITE_KEYS.GOOGLE_LOGIN_ENABLED, isEnabled);
    setToggling(false);
    toast.success(
      isEnabled
        ? "Google Login is now DISABLED — button hidden from users"
        : "Google Login is now ENABLED — button visible to all users"
    );
  };
  ...
  return (
    <div className="glass-card p-6 border border-blue-500/10">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h3 className="font-semibold text-lg">Google Login Control</h3>
          <p className="text-xs text-muted-foreground">Show or hide "Continue with Google" on Login & Signup pages</p>
        </div>
      </div>

      <div className="flex items-center justify-between p-4 bg-secondary/50 rounded-xl border border-border/50">
        <div className="flex-1 min-w-0">
          <p className="font-medium text-sm">Enable Google Sign-In / Sign-Up</p>
          <p className="text-xs text-muted-foreground mt-1">
            Controls the "Continue with Google" button visibility across the entire site
          </p>
          {/* Status badge */}
          <div className="flex items-center gap-2 mt-2">
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold ${isEnabled
                ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/20"
                : "bg-red-500/15 text-red-400 border border-red-500/20"
              }`}>
              <span className={`w-1.5 h-1.5 rounded-full ${isEnabled ? "bg-emerald-400" : "bg-red-400"}`} />
              {isEnabled ? "Enabled — visible to users" : "Disabled — hidden from users"}
            </span>
          </div>
        </div>
        ...
        <button
          ...
          className="ml-4 flex-shrink-0 focus:outline-none disabled:opacity-60"
        >
          {toggling ? (
            ...
            <div className={`relative w-12 h-6 rounded-full transition-all duration-300 ${isEnabled ? "bg-emerald-500 shadow-[0_0_12px_hsl(142_70%_45%/0.4)]" : "bg-secondary border border-border"
              }`}>
              <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow-md transition-all duration-300 ${isEnabled ? "left-6" : "left-0.5"
                }`} />
            </div>
          )}
        </button>
      ...
      <p className="text-xs text-muted-foreground mt-3 px-1">
        ⚠️ The underlying Google OAuth API routes remain active regardless of this toggle.
        Only the frontend button visibility is controlled here.
      </p>
    </div>
  );
};
...
const AdminSettings = ({ user }: { user: any }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 max-w-2xl mx-auto">
      ...
      <GoogleLoginToggle getBool={getBool} toggleBool={toggleBool} settingsLoading={settingsLoading} />
